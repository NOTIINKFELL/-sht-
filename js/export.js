...
function validateExport(data, struct) {
	...
	if (ver == 13) {
		if (data.sht_arr.extra.length < 1)  err("没有灵界发射器的数据");
	};

	//check amount of foc/unfoc shootersets
	//ingore for photogames since their .shts are janky
	if (struct.type == "maingame") {
		let len = data.最大power + 1;
		if (!isNaN(len)) {
			if (struct.f_uf_shooter_split) {
				if (data.sht_arr.focused.length != len) err("低速的发射器数量不正确（应该是"+len+"）");
				if (data.sht_arr.unfocused.length != len) err("高速的发射器数量不正确（应该是"+len+"）");
			};
		};
	};

	if (data.sht_off_cnt < data.real_sht_off_cnt) err("sht_off_cnt不能小于real_sht_off_cnt", true);
	...
	if (cnt != sum) err("real_sht_off_cnt不正确（应该是"+sum+"）");
	...
	if (struct.forced_shtoffarr_len) {
		if (data.sht_off_cnt != struct.forced_shtoffarr_len)
			err(
`ZUN的解析器不太好，期望射击集数组从一个静态偏移开始，
因此sht_off_cnt必须为${struct.forced_shtoffarr_len}。如果你想让它更小，
你可以将real_sht_off_cnt设置为你想要的值 - 空偏移将自动插入，
以匹配所需的数量。如果你想让它更大，这是不可能的`
			);
	};
};
...
function getExportShtArr(struct) {
	...
	let offsets = [];
	let powers = []; // for <TH10
	let 最大power = getLastValid("main", "最大power");
...
function getExportOptPos(struct) {
	...
	let 最大power = getLastValid("main", "最大power");
	let j = 0;
	let max = struct.ver > 12 ? struct.max_opt : shtObject.最大power;
	for (let focused=0; focused<2; focused++) {
		let foc = focused ? "focused" : "unfocused";
