...
function readSht(arr, struct) {
	...
	let i = 0;
	let data = {};
	if (struct.ver == 10 || struct.ver == 10.3) data.最大power = 4; // janky format of a janky game
	...
	while(offset < arr.length) {
		...
		switch(type) {
			...
			case "option_pos":
				...
				let max = struct.ver > 12 ? struct.max_opt : data.最大power;
			...
			case "sht_arr":
				...
				val = readShtArr(arr, offset, data.sht_off, struct.sht_arr, struct.flags_len, struct.flag_size, data.最大power, struct.sht_off_type, data.powers);
			break;
